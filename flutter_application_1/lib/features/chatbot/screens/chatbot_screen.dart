...
class _ChatbotViewState extends State<_ChatbotView> {
  ...
  /// Tracks the last-seen message count so we only auto-scroll when a new
  /// message is actually appended — not on every rebuild from a keystroke,
  /// which would fight the user's own scroll gestures.
  ...
  // ChatProvider seeds the welcome message in its own constructor, so
  // initState intentionally does NOT call startNewConversation() — that
  // would wipe and re-seed the transcript for no reason and briefly flash
  // an empty list on first frame.
    ...
    return Scaffold(
      ...
      body: prov.Consumer<ChatProvider>(
        builder: (context, provider, _) {
          // Only auto-scroll when the message list actually grows. Scrolling
          // on every rebuild — including keystroke-driven rebuilds from the
          // input field — was yanking the viewport while users typed.
