...
class RateServiceScreen extends ConsumerStatefulWidget {
  ...
  const RateServiceScreen({super.key, required this.targetId});
...
class _RateServiceScreenState extends ConsumerState<RateServiceScreen> {
  ...
  Future<void> _submitReview() async {
    ...
    try {
      final targetRef = FirebaseFirestore.instance.collection('users').doc(widget.targetId);
      final newReviewRef = targetRef.collection('reviews').doc();
      
      await FirebaseFirestore.instance.runTransaction((transaction) async {
        final targetDoc = await transaction.get(targetRef);
        if (!targetDoc.exists) throw Exception("Target user not found");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<DocumentSnapshot>(
        future: FirebaseFirestore.instance.collection('users').doc(widget.targetId).get(),
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(targetUser.category ?? '', style: const TextStyle(color: Colors.grey, fontSize: 16)),
