...
class _ClientEditProfileScreenState
  ...
  /// Converts a stored language value (could be 'en', 'English', 'fr', etc.) to a standard code.
  String _mapToLanguageCode(String value) {
  ...
  void _hydrate(ClientModel user) {
    ...
    // Fix: ensure the language is always a valid code
    final storedLang = user.language ?? 'en';
    _language = _mapToLanguageCode(storedLang);

    _notificationsEnabled = user.notificationsEnabled;
    _location = user.location;
  }

  /// Safely returns the first character of a string or '?' if empty/null.
  ...
  Future<void> _save(ClientModel user) async {
    ...
    try {
      ...
      final updated = ClientModel(
        ...
        language: _language, // now always a valid code
        ...
        children: [
          ...
          DropdownButtonFormField<String>(
            value: _language.isNotEmpty &&
                ['en', 'fr', 'ar'].contains(_language)
                ? _language
                : null, // fallback to null if invalid (dropdown will show no selection)
