...
class AIService {
  ...
  final String _systemPrompt =
      """You are a helpful and friendly AI home service advisor. Your role is to:
1. Help users identify and understand home repair issuesr
2. Analyze photos/images of problems when provided
3. Determine whether DIY is safe or professional help is needed
4. Provide step-by-step 4uidance for safe DIY fixes
...
If user asks about something unrelated to home services, politely redirect: 'I'm here to help with home repair issues. Is there something home-related I can help with?'""";
  ...
  Future<MessageModel> sendMessage(List<MessageModel> history) async {
    ...
    if (apiKey == null || apiKey.isEmpty) {
      throw Exception(
        'API Configuration Error: GOOGLE_GEMINI_API_KEY is missing.',
      );
    }
    ...
    try {
      ...
      for (var c in chatHistory) {
        if (compressedHistory.isNotEmpty &&
            compressedHistory.last.role == c.role) {
      ...
      final response = await model
          .generateContent(compressedHistory)
