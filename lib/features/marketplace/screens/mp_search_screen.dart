...
class MpSearchScreen extends ConsumerStatefulWidget {
  const MpSearchScreen({super.key});

  @override
  ConsumerState<MpSearchScreen> createState() => _MpSearchScreenState();
}

class _MpSearchScreenState extends ConsumerState<MpSearchScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          // 1. Map View (Using Firestore Query with the where clause)
          StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                .collection('users')
                .where('accountType', isEqualTo: 'marketplace')
                .where('uid', isNotEqualTo: currentUserId) // CRITICAL: Exclude self
                .snapshots(),
                ...
                for (var doc in snapshot.data!.docs) {
                  ...
                  if (lat != null && lng != null) {
                    markers.add(Marker(
                      ...
                      width: 30, height: 30,
                      child: const Icon(Icons.store, color: AppColors.accentOrange, size: 30),
                    ));
                  }
                }
              }
