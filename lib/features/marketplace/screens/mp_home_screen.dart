...
// ---------------------------------------------------------------------------
final _mpRecentConversationsProvider =
...
// Screen
// ---------------------------------------------------------------------------
class MpHomeScreen extends ConsumerWidget {
  const MpHomeScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final uid = FirebaseAuth.instance.currentUser?.uid ?? '';
    final conversations = ref.watch(_mpRecentConversationsProvider(uid));
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            // ── Search Banner ─────────────────────────────────
            _MpSearchBanner(uid: uid),
            const SizedBox(height: 20),

            // ── Shop Status ───────────────────────────────────
            _ShopStatusCard(uid: uid),
            const SizedBox(height: 20),
      ...
      bottomNavigationBar: _MpBottomNav(currentIndex: 0),
    );
  }
}

// ---------------------------------------------------------------------------
// Search Banner for MP
// ---------------------------------------------------------------------------
class _MpSearchBanner extends ConsumerWidget {
  final String uid;
  const _MpSearchBanner({required this.uid});
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Discover other stores and grow your network',
              style: AppTextStyles.bodyMedium.copyWith(color: AppColors.softGray)),
          const SizedBox(height: 14),
          GestureDetector(
            onTap: () => context.push('/search-filter?target=marketplace&excludeId=$uid'),
            child: Container(
              decoration: BoxDecoration(
...
// Shop Status Card
// ---------------------------------------------------------------------------
class _ShopStatusCard extends ConsumerStatefulWidget {
  final String uid;
  const _ShopStatusCard({required this.uid});

  @override
  ConsumerState<_ShopStatusCard> createState() => _ShopStatusCardState();
}

class _ShopStatusCardState extends ConsumerState<_ShopStatusCard> {
  bool _isOpen = false;
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    if (mounted) {
      setState(() {
        _isOpen = (doc.data()?['isOpen'] as bool?) ?? false;
        _loading = false;
      });
    }
  }
...
class _ConversationTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final participants =
        (conversation['participantIds'] as List<dynamic>?)?.cast<String>() ?? [];
    final otherId =
...
class _MpBottomNav extends ConsumerWidget {
  ...
  const _MpBottomNav({required this.currentIndex});
        ...
        child: Row(
          ...
          children: [
            ...
            _NavItem(1, Icons.search, 'Search MP', currentIndex,
                () => context.push('/search-filter?target=marketplace&excludeId=$uid')),
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      child: Column(
        ...
        children: [
          Icon(icon,
              color: selected ? AppColors.electricBlue : AppColors.softGray,
              size: 22),
          const SizedBox(height: 4),
          Text(label,
              style: TextStyle(
